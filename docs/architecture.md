...
4. **Local-first auditability.** Conversation and execution state live in SQLite; lifecycle events
   remain available in JSONL; OpenAI responses default to `store=false`.
...
- **Operational memory:** SQLite checkpoints and events plus JSONL export for debugging, recovery,
  evaluation, replay tooling, and cost tracking.
